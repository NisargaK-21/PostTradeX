...
export default function AuditPage() {
  ...
  useEffect(() => {
    axios
      ...
      .then(res => {
        const auditLogs = res.data.map(t => ({
          action: t.status === "SETTLED" ? "SETTLEMENT_COMPLETED" : "TRADE_RECORDED",
          tradeId: t.tradeId,
          timestamp: new Date((t.settledAt || t.createdAt) * 1000).toLocaleString(),
        }));
        setLogs(auditLogs);
      })
      .catch(console.error);
